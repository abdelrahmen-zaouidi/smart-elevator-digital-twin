...
#define OUT_F1_UP_PIN         10
#define OUT_F1_DOWN_PIN       11
#define OUT_F2_UP_PIN         12
#define OUT_F2_DOWN_PIN       13
...
//   Second press while emergency-> clearEmergencyAndResume()
...
void enterEmergency(const char* reason) {
  // If we were mid-trip, memorise where we were so a clear/RESET can
  // resume the same trip instead of pretending we are on the origin floor.
...
void clearEmergencyAndResume(const char* source) {
  ...
  if (resumePendingMove
      && resumeTargetFloor >= 0 && resumeTargetFloor < NUM_FLOORS
      && resumeSegmentTargetFloor >= 0 && resumeSegmentTargetFloor < NUM_FLOORS
      && resumeStepsRemaining > 0) {

    targetFloor       = resumeTargetFloor;
    segmentTargetFloor = resumeSegmentTargetFloor;
    dispatchDirection = resumeDirectionUp ? 1 : -1;
    stepsToDeliver    = resumeStepsRemaining;
    stepsDelivered    = 0;
    // V6.1: restore the within-segment offset so floor-boundary detection
    // resumes correctly. Without this currentFloor would lag by up to one
    // floor after resume.
    stepsSinceLastBoundary = resumeStepsSinceLastBoundary;
    moveStartMs       = millis();

    // Safety budget: one full-floor budget per floor of the remaining
    // distance, with a one-floor minimum so short tail-end resumes don't
    // immediately trip the watchdog.
    unsigned long floorsRemaining = (resumeStepsRemaining + STEPS_PER_FLOOR - 1) / STEPS_PER_FLOOR;
    if (floorsRemaining < 1) floorsRemaining = 1;
    maxMoveTimeMs     = floorsRemaining * MAX_FLOOR_TRAVEL_MS;
    lastFanActivityMs = moveStartMs;

    if (resumeDirectionUp) { setDirectionUp();   transitionTo(ST_MOVING_UP); }
    else                   { setDirectionDown(); transitionTo(ST_MOVING_DOWN); }
    setFaultReason("NONE");
    lcdShowMessageTemporary("RESET DONE", "Resuming trip", "Target restored", "Monitor motion", LCD_TEMP_MESSAGE_DURATION_MS);

    Serial.print("[");
    Serial.print(source);
    Serial.print("] EMERGENCY cleared, resuming trip -> floor ");
  ...
  // No memorised trip: plain reset to IDLE.
  targetFloor    = -1;
  segmentTargetFloor = -1;
  dispatchDirection = 0;
  stepsToDeliver = 0;
  ...
  setFaultReason("NONE");
  lcdShowMessageTemporary("RESET DONE", "State: IDLE", "Current floor kept", "System ready", LCD_TEMP_MESSAGE_DURATION_MS);
  Serial.print("[");
  Serial.print(source);
  Serial.println("] EMERGENCY cleared -> IDLE (current floor kept)");
  publishNow = true;
}
...
void publishTelemetry() {
  ...
  char payload[JSON_DOC_CAPACITY];
...
void onMqttMessage(char* topic, byte* payload, unsigned int length) {
  ...
  if (body.length() == 1 && body[0] >= '0' && body[0] <= '3') {
    handleFloorRequest(body[0] - '0', "MQTT");
    return;
  }
