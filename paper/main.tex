...
\author{Abderrahmane~Zaouidi,
        ...
        and~Mounir~Bouhedda%
\thanks{The authors are with the Laboratory of Advanced Electronic Systems (LSEA), University of M\'ed\'ea, M\'ed\'ea, Algeria (e-mails: abdelrahmenzaouidi@gmail.com; mohamed.bengherbia26@gmail.com; bouhedda.mounir@univ-medea.dz).}%
\thanks{Corresponding author: Abderrahmane Zaouidi.}}

\markboth{Draft journal article prepared from the master's thesis, July 2026}%
{Zaouidi \MakeLowercase{\textit{et al.}}: Safety-Gated Digital Twin Architecture for Smart-Elevator Supervision}

\maketitle

\begin{abstract}
Connected supervision makes elevators easier to monitor, but it turns brokers, dashboards, workflow engines, and twin APIs into part of the safety boundary, where the decisive risk is command authority: who or what may move a safety-critical machine. We present an architecture and software-validation study of a laboratory-scale smart-elevator digital twin that addresses this risk through a safety-gated command path. Eclipse Ditto holds the authoritative state; every operator or workflow action becomes command \emph{intent} that a deterministic rule-based gate must admit before it is persisted for audit and forwarded to the device, which still applies firmware interlocks; a rejected command is logged yet produces zero twin writes. The self-hosted platform couples a four-floor ESP32-S3 prototype and an authenticated simulator with MQTT/TLS, a bridge, PostgreSQL/TimescaleDB, bounded n8n workflows, an adaptive dispatch engine, and a SCADA dashboard; workflows and an optional language model explain and recommend but never hold command authority, and the machine-learning dispatch challenger remains a non-authoritative shadow. Validation is reported at the software and integration levels: 127 automated assertions pass; broker authentication, access-control lists, and TLS are exercised; and a 57-command live campaign yields 28 accepted and 29 rejected commands, the reject-with-zero-write invariant holding on all 29 rejections, database-verified. The gate admits a command in a median of 5.7~$\mu$s, about 5.3~$\mu$s over a direct publish, so the safety discipline imposes negligible computational cost on command logic. Floor position is tracked by calibrated open-loop step counting; closed-loop sensing, instrumented latency, and embedded-resource profiling remain future work.
\end{abstract}
...
Elevator IoT / predictive \mbox{maintenance} \cite{vanDinter2022predictiveMaintenance,zonta2020predictiveMaintenance} & P & -- & -- & -- & -- & Condition monitoring and failure prediction \\
Runtime assurance / shielding / \mbox{supervisory} control \cite{sha2001simplex,alshiekh2018shielding,ramadge1989supervisoryControl} & -- & P & Y & -- & P & Keeping an untrusted controller within a safety envelope \\
...
\centering
\footnotesize
\begin{tabularx}{\textwidth}{@{}p{0.18\textwidth}p{0.21\textwidth}Yp{0.18\textwidth}@{}}
\toprule
Layer & Owns & Must not own & Reliability\slash security \mbox{rationale} \\
\midrule
...
PostgreSQL\slash TimescaleDB & Telemetry history, audit, command decisions, notifications, maintenance and dispatch records. & Current operational authority. & Enables traceability and analytics without making stale database rows control the elevator. \\
...
Next.js SCADA \mbox{dashboard} & Operator visualization, history views, command-request UI, server-side API calls. & Direct MQTT publication or client-side secrets. & Prevents browser bypass of Ditto, safety gate, and audit logging. \\
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{tikzpicture}[
  ...
  gate/.style={box, very thick, text width=21mm},
  term/.style={box, text width=22mm},
  lbl/.style={font=\scriptsize\itshape, inner sep=1.5pt},
  node distance=5.5mm and 4.2mm,
]
% main accept pipeline (left to right)
\node[box, text width=16mm] (op) {Operator or workflow};
\node[box, right=of op] (api) {Dashboard API\\{\scriptsize\ttfamily POST /api/\allowbreak commands}};
\node[gate, right=of api] (gate) {Deterministic safety gate};
\node[box, right=10mm of gate] (ditto) {Ditto intent\\{\scriptsize\ttfamily control.\allowbreak pending\_\allowbreak command}};
\node[box, right=of ditto, text width=16mm] (bridge) {Bridge\\(SSE / poller)};
\node[box, right=of bridge] (mqtt) {MQTT topic\\{\scriptsize\ttfamily elevator/\allowbreak\{id\}/\allowbreak commands}};
...
\node[box, text width=44mm, below=9mm of gate, anchor=north west, xshift=-6mm] (reject)
  {Reject: {\scriptsize\ttfamily control\_\allowbreak command\_\allowbreak log} $+$ {\scriptsize\ttfamily audit\_\allowbreak log} written,\\
   {\scriptsize\ttfamily ditto\_\allowbreak write\_\allowbreak status\,=\,SKIPPED}\\(zero Ditto write)};
\draw[->] (gate.south) -- node[right, lbl] {reject} (gate.south |- reject.north);
% interlock note under device
\node[lbl, below=4mm of dev, text width=26mm, align=center] (lock)
  {floor bounds, \mbox{e-stop}, overload, door, lockout};
...
Control workflow & Operator or workflow command request. & Run deterministic command safety gate and branch accepted/rejected paths. & \artifact{control\_\allowbreak command\_\allowbreak log}, \artifact{audit\_log}, Ditto writes only if accepted. \\
...
Maintenance workflow & Wear/risk threshold, repeated incident, scheduled review. & Create or update work-order state using rule-based evidence. & \artifact{maintenance\_\allowbreak schedule}, \artifact{maintenance\_\allowbreak work\_\allowbreak orders}. \\
Notification workflow & Alert or outbox due event. & Build dedupe key, select channels, retry or mark failed/sent. & \artifact{notification\_\allowbreak outbox}. \\
Optimization\slash audit \mbox{workflow} & Dispatch cycle or audit event. & Record dispatch decisions, policy changes, reward/proxy outcomes, workflow activity. & Dispatch/audit tables and dashboard history APIs. \\
\bottomrule
\end{tabularx}
\end{table*}
...
\caption{Reduced-scale four-floor, single-cabin ESP32-S3 elevator prototype (annotated). Floor position is tracked functionally by calibrated step counting; temperature, vibration, and load are simulated through ADC potentiometers. KY-024 and SPDT switches are design provisions for future closed-loop floor/door confirmation.}
\label{fig:prototype}
\end{figure}

Two aspects of Table~\ref{tab:prototype-specs} should be retained. First, the backend stack is exercised against a real electromechanical plant with real actuation and timing, not only against a software model: the prototype and the simulator implement the same MQTT contract, so integration evidence gathered with either endpoint applies to the same interface. Second, the table itself encodes the evidence boundary of contribution three---floor position is tracked functionally in open loop, and safety telemetry is potentiometer-simulated---so the reader can see, per subsystem, which claims are physical and which are not.

The deployed firmware implements a finite-state machine with idle, door-opening, door-wait, door-closing, moving-up, moving-down, arrival, error-stop, and emergency states. Remote commands are not direct GPIO operations; they are parsed into state-machine requests and checked against firmware-side interlocks.
...
The firmware publishes JSON telemetry to \artifact{elevator/\allowbreak\mqttid/\allowbreak telemetry}, events to \artifact{elevator/\allowbreak\mqttid/\allowbreak events}, retained status to \artifact{elevator/\allowbreak\mqttid/\allowbreak status}, and subscribes to \artifact{elevator/\allowbreak\mqttid/\allowbreak commands}. Firmware telemetry uses a Ditto-aligned feature envelope, and the repository configuration sets a 3000 ms periodic telemetry interval when not moving, a 5000 ms status interval, and position-oriented updates during motion. These are configured design parameters unless instrumented captures are added.
...
\begin{tikzpicture}[font=\footnotesize,
  blk/.style={draw, rounded corners=2pt, align=left, inner sep=4.5pt, text width=0.82\columnwidth},
  node distance=1.8mm,
]
...
&{}\wedge \operatorname{riskWithinLimit}(c,T) \\
&{}\wedge \operatorname{dittoReachable}(T).
\end{aligned}
\end{equation}
...
Table~\ref{tab:gate-rules} lists the principal rule families. The final software evidence confirms that rejected decisions carry no Ditto writes. In the live command-path runs, this property was not only unit-tested but also observed in database rows: every rejected command had \artifact{ditto\_write\_status = SKIPPED}.

The rule families in Table~\ref{tab:gate-rules} operationalize Eq.~\eqref{eq:gate} and span three kinds of protection: syntactic validity (catalog, fields, bounds), plant-state safety (mode, interlocks, freshness), and accountability (source, reason, confirmation, cooldown). The zero-write invariant in the final row is the architectural anchor: it turns every other family's rejection into a guaranteed non-event for the twin. Section~\ref{sec:eval} shows that evaluating the full predicate costs single-digit microseconds, so this breadth of checking does not translate into a latency argument against it.
...
Human confirmation & Emergency stop, reset, lockdown, \mbox{release}, maintenance-mode transitions. & Prevents accidental state-changing actions and forces explicit operator intent. \\
Twin freshness and \mbox{availability} & Stale twin; Ditto unavailable; missing safety snapshot. & Avoids acting on old or incomplete operational state. \\
...
Bounds and policy \mbox{validity} & Target floor outside 0--3; unknown dispatch policy; invalid policy parameters. & Preserves physical and logical limits of the prototype. \\
...
\caption{SCADA command center. Operator commands post to \artifact{/api/commands} and pass the deterministic safety gate; recent command decisions and their accept/reject reasons are shown.}
\label{fig:dashboard}
\end{figure}
...
\centering
\footnotesize
\begin{tabularx}{\textwidth}{@{}p{0.20\textwidth}p{0.13\textwidth}Yp{0.19\textwidth}@{}}
\toprule
Subsystem & Status & Evidence & Boundary \\
\midrule
MQTT topic convention & PASS & \artifact{evidence/\allowbreak mqtt/\allowbreak topic\_\allowbreak validator\_\allowbreak output.txt}: scanned 266 files, no unexpected legacy topic patterns, expected canonical topics present. & Software/configuration evidence. \\
MQTT authentication and ACLs & PASS & \artifact{acl\_\allowbreak security\_\allowbreak tests.txt}: anonymous and wrong-password connections denied; authenticated bridge subscriber received telemetry. & Broker-side evidence. \\
TLS publish/subscribe & PASS & Workspace \artifact{evidence/\allowbreak mqtt/\allowbreak 05-roundtrip-tls.txt}: TLS publish exit 0 and subscriber receive exit 0 on 8883. & Field RTT not timed; loopback RTT in \mbox{Table~\ref{tab:metrics}}. \\
...
Command safety gate & PASS & \artifact{command\_\allowbreak safety\_\allowbreak gate\_\allowbreak test\_\allowbreak output.txt}: 33/33 assertions; live command rows show rejected commands with \artifact{SKIPPED} writes. & Deterministic software gate. \\
...
Simulator regression & PASS & \artifact{simulator\_\allowbreak pytest\_\allowbreak output.txt}: 33/33 pytest items in 0.31 s. & Simulator software evidence, not physical hardware proof. \\
...
End-to-end command path & PASS & Nine-decision run (2026-06-13) plus 57-command campaign (2026-07-05): 28 accepted, 29 rejected, all 29 rejections \artifact{SKIPPED}, database-verified (\artifact{evidence/\allowbreak command-campaign/}). & Integration-level functional coverage, not fault injection or soak testing; device endpoint was authenticated \mbox{simulator}. \\
...
Two live command-path executions were captured. The first, part of the 2026-06-13 validation package, records nine decisions: an \artifact{EMERGENCY\_STOP} without confirmation was rejected, a confirmed \artifact{EMERGENCY\_STOP} was accepted and forwarded, a \artifact{MOVE\_TO\_FLOOR} request during unsafe state was rejected, \artifact{RESET\_EMERGENCY} was accepted, unknown dispatch policies were rejected, and valid \artifact{SET\_DISPATCH\_POLICY} commands were accepted and adopted by the simulator.

The second is a scripted 57-command campaign (2026-07-05) driven through \artifact{POST /api/commands} against the full stack---dashboard, gate, Ditto, bridge, Mosquitto with authentication and ACLs, TimescaleDB, and the authenticated simulator with anomaly injection disabled for reproducibility. The campaign mixed accepted and rejected classes across eight live-exercisable rule families of Table~\ref{tab:gate-rules}: unknown commands, missing required fields, missing operator reason, missing human confirmation, mode guards traversed through complete emergency-stop, lockdown, and maintenance cycles with recovery, floor bounds, payload and policy validity, and duplicate/cooldown. Of 57 commands, 28 were accepted and 29 rejected, and every decision matched the catalog-predicted outcome. All 28 accepted commands completed their Ditto writes; all 29 rejected commands carried \artifact{ditto\_write\_status = SKIPPED}, and an independent export of \artifact{control\_command\_log} confirms both counts with zero invariant violations (\artifact{evidence/\allowbreak command-campaign/}). An initial campaign execution, retained in the same evidence directory, additionally exercised a plant-state interlock live: four \artifact{MOVE\_TO\_FLOOR} requests issued while the simulator autonomously served passengers were rejected with \artifact{door is open}.

These campaigns provide integration-level functional coverage of the live accept and reject branches per rule family; they are not long-duration robustness or fault-injection experiments. Families that require an unhealthy plant or infrastructure faults---twin staleness, Ditto unavailability, overload and forced-entry interlocks, risk-threshold caps---remain covered by the 33/33 gate assertions, and the 20{,}000-samples-per-class microbenchmarks below quantify decision cost. Fault-injection and long-duration soak campaigns remain future work.
...
Command safety-gate \mbox{validation} & 33/33 assertions; rejected commands carry zero Ditto writes. Suite duration is test-runner time, not latency. & Measured (functional). \\
...
Live command evidence covers the healthy-plant configuration. & 57-command campaign across eight rule families: 28 accepted, 29 rejected, zero-write invariant on 29/29 rejections (database-verified); a prior execution also exercised the door-open interlock live. & Fault-injection campaigns (broker or twin outage, stale twin, overload) and long-duration soak runs. \\
...
The evidence supports these claims at the software and integration levels rather than as certified physical validation. A total of 127 automated assertions passed, MQTT authentication and ACLs were exercised, TLS publish/subscribe was demonstrated, and Ditto and database exports were captured. On the live stack, a nine-decision run and a 57-command campaign across eight gate rule families recorded 28 accepted and 29 rejected commands, with every rejection carrying zero Ditto writes---database-verified, integration-level functional coverage of the command path. The deterministic decision paths were also quantified: the safety gate admits a command in a median of 5.7~$\mu$s and adds about 5.3~$\mu$s over a direct publish, so the discipline imposes negligible computational cost on command logic. Floor position is tracked functionally by calibrated step counting; closed-loop sensor confirmation, fault-injection and soak campaigns, instrumented end-to-end latency, and embedded-resource profiling remain future work. Within these boundaries, the platform is offered as a safety-first, auditable, bounded-autonomy architecture for smart-elevator digital twins.

\section*{Data and Artifact Availability}

The implementation and the thesis evidence package dated 2026-06-13 are maintained in the project repository at \url{https://github.com/abdelrahmen-zaouidi/smart-elevator-digital-twin} (Apache License 2.0). Secrets, passwords, and private environment files are excluded. A camera-ready submission provides a sanitized artifact bundle or repository snapshot with the evidence paths listed in this paper.
...
The authors acknowledge LSEA, University of M\'ed\'ea, for the academic supervision and laboratory context of this work.

% Balance the two columns of reference entries on the last page.
\IEEEtriggeratref{22}
