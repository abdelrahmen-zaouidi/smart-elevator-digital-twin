...
\author{Zaouidi~Abderrahmane,
        ...
        and Mounir~Bouhedda%
% TODO(human): confirm official lab name + accents. The acronym ``LSEA'' does not match ``Laboratoire Signaux et Syst\`emes'' (that would be LSS); provide the correct expansion.
\thanks{The authors are with the Laboratoire Signaux et Syst\`emes (LSEA), University of M\'ed\'ea, M\'ed\'ea, Algeria.}%
% TODO(human): insert real corresponding-author e-mail addresses (do not invent).
\thanks{Corresponding authors: Zaouidi Abderrahmane and Mounir Bouhedda. E-mails: [AUTHOR EMAIL], [SENIOR AUTHOR EMAIL].}}
...
\centering
\footnotesize
\begin{tabularx}{\textwidth}{@{}p{0.16\textwidth}p{0.22\textwidth}Yp{0.18\textwidth}@{}}
\toprule
...
\begin{tabular}{>{\centering\arraybackslash}p{0.12\textwidth}
                ...
                >{\centering\arraybackslash}p{0.12\textwidth}
                >{\centering\arraybackslash}p{0.12\textwidth}}
\textbf{Operator or agent} &
\textbf{Dashboard API} &
\textbf{Deterministic safety gate} &
\textbf{Audit and database} &
\textbf{Ditto command intent} &
\textbf{Bridge} &
\textbf{MQTT device topic} \\
\artifact{POST /api/commands} &
fetch live twin state &
allow-list, source, reason, confirmation, risk, mode, freshness &
\artifact{control\_command\_log} and \artifact{audit\_log} &
\artifact{control.pending\_command} &
SSE or poller reconciles accepted intent &
\artifact{elevator/\{id\}/commands}
\end{tabular}

\vspace{3pt}
$\Downarrow$ firmware/simulator local interlocks: floor bounds, emergency stop, overload, door state, lockout, malformed command rejection
\end{minipage}}
\caption{Safety-gated command path. The browser, workflows, and optional LLM never publish device commands directly. Accepted intent is persisted in Ditto and forwarded by the bridge; rejected commands produce no Ditto writes.}
...
\begin{equation}
\begin{split}
accepted(c) ={}& allowlisted(c) \land valid(c) \land authorized(c) \\
& \land fresh(T) \land safeMode(T) \land safeInterlocks(T) \\
& \land riskWithinLimit(c,T) \land dittoReachable .
\end{split}
\label{eq:gate}
\end{equation}

Equation~\eqref{eq:gate} is a compact representation of the admission logic; the implementation records all rejection reasons rather than returning only the first failure.
