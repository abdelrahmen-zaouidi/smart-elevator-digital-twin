...
\begin{abstract}
Connected supervision makes elevators easier to monitor, but it turns brokers, dashboards, workflow engines, and twin APIs into part of the safety boundary, where the decisive risk is command authority: who or what may move a safety-critical machine. We present a laboratory-scale smart-elevator digital twin that resolves this through a safety-gated command path. Every operator or agent action becomes command \emph{intent} on an authoritative Eclipse Ditto twin, is admitted by a deterministic rule-based gate, persisted for audit, and only then forwarded to the device, which still applies firmware interlocks; a rejected command produces zero twin writes. The platform is self-hosted on a four-floor ESP32-S3 prototype with authenticated MQTT/TLS (Mosquitto), a Node.js bridge, PostgreSQL/TimescaleDB, n8n workflows, an adaptive dispatch engine, and a Next.js SCADA dashboard, with every autonomy layer bounded: workflows and an optional language model explain and recommend but never hold safety authority. Across 127 passing software assertions and a live command-path run, the gate admits a command in a median of 5.7~$\mu$s, so the safety discipline costs essentially nothing.
\end{abstract}
