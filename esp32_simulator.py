...
class ElevatorPhysics:
    ...
    def _phase_idle(self, dt: float) -> None:
        ...
        if self.door_timer_s <= 0 and self._has_pending_call():
    ...
    def _phase_decelerating(self, dt: float) -> None:
        ...
        self.speed_ms = max(0.0, self.speed_ms - self.cfg.decel_ms2 * dt)
    ...
    def _check_arrival(self) -> None:
        ...
        braking_distance = (self.speed_ms ** 2) / (2 * max(self.cfg.decel_ms2, 1e-6))
...
# MQTT
# ──────────────────────────────────────────────────────────────────────────────
def make_mqtt_client(cfg: SimConfig) -> mqtt.Client:
