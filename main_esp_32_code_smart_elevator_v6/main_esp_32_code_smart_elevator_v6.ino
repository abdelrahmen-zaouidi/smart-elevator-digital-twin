...
// =====================================================
// LCD 1604A I2C HMI CONFIGURATION
// -----------------------------------------------------
// Requires the Arduino "LiquidCrystal I2C" compatible library:
//   Sketch > Include Library > Manage Libraries > search LiquidCrystal_I2C
//
// Electrical warning:
// Many PCF8574 LCD backpacks are powered from 5 V and include SDA/SCL pull-up
// resistors to that 5 V rail. ESP32-S3 GPIO is 3.3 V logic and is NOT 5 V
// tolerant. If your backpack pulls I2C up to 5 V, use a bidirectional level
// shifter or move the pull-ups to 3.3 V. Do not connect 5 V I2C lines directly
// to ESP32-S3 GPIO.
// =====================================================
#define ENABLE_LCD                         1
...
// =====================================================
// ESP32-S3 SMART ELEVATOR PROTOTYPE  -  V6.1  (continuous multi-floor)
// -----------------------------------------------------
// V6 introduced step-count positioning (over time-based positioning) to
// eliminate loop-jitter-induced drift. V6.1 builds on that and fixes the
// "slows/stops at every floor" symptom by replacing per-floor segmented
// travel with a single multi-floor trip plan:
//
//   * startMovementSegmentTowardTarget() now sets stepsToDeliver to the
//     FULL distance (STEPS_PER_FLOOR * floors_to_travel), so the accel
//     ramp engages once at trip start and the decel ramp once at the
//     actual stop. Pass-through floors are crossed at cruise speed.
//   * tickMoving() detects floor-boundary crossings via
//     stepsSinceLastBoundary and updates currentFloor in real time. If a
//     same-direction request appears at the floor about to be entered,
//     redirectTripToNextFloor() shortens the trip so the cabin lands
//     there with a clean decel ramp.
//   * The emergency-stop resume path saves and restores
//     stepsSinceLastBoundary so currentFloor doesn't drift after a
//     mid-trip clear.
//
// V6 features inherited:
//   * STEPS_PER_FLOOR replaces FLOOR_TRAVEL_TIME_MS. Arrival is decided
//     by step pulses actually delivered, not elapsed wall-clock time.
//   * Acceleration / deceleration ramps anchored to step counts
//     (STEP_ACCEL_STEPS / STEP_DECEL_STEPS).
//   * publishTelemetry() and the periodic status print are skipped while
//     MOVING. mqttClient.loop() still runs so EMERGENCY_STOP / RESET
//     are honoured.
//   * MAX_FLOOR_TRAVEL_MS provides a wall-clock safety timeout that
//     trips ERROR_STOP if step delivery stalls.
//   * Each arrival logs a [CALIB] line for tuning STEPS_PER_FLOOR.
//
// V5 features (FSM, request table, fan AUTO/MANUAL, EMERGENCY state) are
// preserved unchanged. MQTT/Ditto envelope is the same — dashboard does
// not need any change.
// =====================================================

// =====================================================
// V6.2  -  SOFTWARE SAFETY / RELIABILITY REFACTOR  (no new hardware required)
// -----------------------------------------------------
// This pass applied software-only, hardware-compatible fixes. No pin map,
// stepper-signalling scheme, door timing, relay/buzzer logic, ADC mapping or
// MQTT envelope was changed, so existing wiring and the dashboard keep working.
//
// Changed in this pass:
//   * J1/J2 Credentials moved behind an optional gitignored "secrets.h"
//           (SECRET_* macros); current values remain as in-file fallbacks.
//           Optional MQTT username/password is used when configured.
//   * C2    SAFETY: clearing an emergency NO LONGER auto-resumes the trip.
//           The car returns to a safe IDLE and requires a fresh, deliberate
//           floor request before moving. (Auto-resume from an E-stop is unsafe.)
//   * F1    transitionTo() now validates transitions against a legality table
//           and logs [STATE][ILLEGAL] on anomalies (fail-open by default; flip
//           ALLOW_ILLEGAL_TRANSITIONS to 0 for fail-closed after bench testing).
//   * F3    SECURITY: securityLocked is now enforced (blocks new requests and
//           dispatch) and controllable via MQTT SECURITY_LOCK / SECURITY_UNLOCK.
//   * H1    A compact cabin position update is published at trip start and at
//           each floor-boundary crossing while MOVING (PUBLISH_POSITION_WHILE_
//           MOVING) so the twin shows live motion instead of frozen telemetry.
//   * Hygiene: removed dead var, added redirectTrip() prototype, made the
//           telemetry payload buffer static (off-stack), replaced the per-
//           message Arduino String with a fixed buffer (no heap churn).
//
// DEFERRED — require new hardware and/or a motion-core rework, and CANNOT be
// validated without the physical rig. These remain as known limitations:
//   * [C1] E-stop is software-polled, not a hardwired safety circuit. Add a
//          driver ENABLE line / contactor dropped in HARDWARE by the E-stop.
//   * [C3] Door "closed" is time-based, not proven. Add closed/open limit
//          switches and gate all motion on a real "door closed" signal.
//   * [C4] No door obstruction / safety-edge detection (add sensor + reopen).
//   * [C5/D2] OPEN-LOOP POSITION: floors are dead-reckoned from step counts
//          with no feedback and no homing. Add per-floor sensors (hall/IR) +
//          a homing routine; confirm floor alignment before opening the door.
//   * [D1] Temperature/vibration/load are SIMULATED potentiometers, not real
//          sensors — predictive-maintenance data is not physically meaningful
//          until real sensors (e.g. DS18B20 / MPU6050 / HX711) replace them.
//   * [E1/C6] Stepping is bit-banged with blocking delayMicroseconds() inside
//          the super-loop, so motion shares timing with comms and a network
//          stall can pause the pulse train. Move step generation to a hardware
//          peripheral (RMT/MCPWM/timer) to decouple motion and enable full
//          streaming telemetry during travel.
//   * [E2/E3] DIR/STEP use an INPUT/high-Z "tri-state" trick that relies on the
//          driver's pulls and is 3.3V-into-5V marginal; move to push-pull
//          outputs (level-shifted) plus a managed ENABLE.
//   * [J3] No NVS persistence (counters/position reset on reboot); position
//          persistence is only meaningful once homing (C5) exists.
//   * [B2] Inbound MQTT commands are handled re-entrantly mid-motion; a small
//          command queue applied at tick boundaries would improve determinism.
// =====================================================
...
// Acoustic alert output. GPIO47 is a non-strap output on the right header.
...
#if ENABLE_LCD && ( \
    LCD_SDA_PIN == STEP_PIN || LCD_SDA_PIN == DIR_PIN || LCD_SDA_PIN == DOOR_IN1_PIN || LCD_SDA_PIN == DOOR_IN2_PIN || LCD_SDA_PIN == DOOR_EN_PIN || \
    LCD_SDA_PIN == FAN_RELAY_PIN || LCD_SDA_PIN == OUT_F0_UP_PIN || LCD_SDA_PIN == OUT_F1_UP_PIN || LCD_SDA_PIN == OUT_F1_DOWN_PIN || \
    LCD_SDA_PIN == OUT_F2_UP_PIN || LCD_SDA_PIN == OUT_F2_DOWN_PIN || LCD_SDA_PIN == OUT_F3_DOWN_PIN || LCD_SDA_PIN == CABIN_F0_PIN || \
    LCD_SDA_PIN == CABIN_F1_PIN || LCD_SDA_PIN == CABIN_F2_PIN || LCD_SDA_PIN == CABIN_F3_PIN || LCD_SDA_PIN == EMERGENCY_STOP_PIN || \
    LCD_SDA_PIN == SIM_TEMP_ADC_PIN || LCD_SDA_PIN == SIM_VIB_ADC_PIN || LCD_SDA_PIN == SIM_LOAD_ADC_PIN || LCD_SDA_PIN == BUZZER_PIN || \
    LCD_SCL_PIN == STEP_PIN || LCD_SCL_PIN == DIR_PIN || LCD_SCL_PIN == DOOR_IN1_PIN || LCD_SCL_PIN == DOOR_IN2_PIN || LCD_SCL_PIN == DOOR_EN_PIN || \
    LCD_SCL_PIN == FAN_RELAY_PIN || LCD_SCL_PIN == OUT_F0_UP_PIN || LCD_SCL_PIN == OUT_F1_UP_PIN || LCD_SCL_PIN == OUT_F1_DOWN_PIN || \
    LCD_SCL_PIN == OUT_F2_UP_PIN || LCD_SCL_PIN == OUT_F2_DOWN_PIN || LCD_SCL_PIN == OUT_F3_DOWN_PIN || LCD_SCL_PIN == CABIN_F0_PIN || \
    LCD_SCL_PIN == CABIN_F1_PIN || LCD_SCL_PIN == CABIN_F2_PIN || LCD_SCL_PIN == CABIN_F3_PIN || LCD_SCL_PIN == EMERGENCY_STOP_PIN || \
    LCD_SCL_PIN == SIM_TEMP_ADC_PIN || LCD_SCL_PIN == SIM_VIB_ADC_PIN || LCD_SCL_PIN == SIM_LOAD_ADC_PIN || LCD_SCL_PIN == BUZZER_PIN)
#error "LCD I2C pins conflict with an existing elevator GPIO assignment."
#endif


// =====================================================
// NETWORK / MQTT CONFIGURATION  --  EDIT FOR YOUR SITE
// -----------------------------------------------------
// SECRETS HANDLING (J1/J2):
// Real credentials should NOT live in version-controlled source. Create a file
// named "secrets.h" next to this sketch (and add it to .gitignore) that
// #defines the SECRET_* macros below. If that file is present it overrides the
// in-file fallbacks; if it is absent the sketch still builds with the fallbacks
// so single-file Arduino builds keep working. After moving the real values into
// secrets.h, scrub the old plaintext password from git history and rotate it.
//
// Example secrets.h:
...
//const char* WIFI_SSID     = "Abderrahmane’s iphone";
...
const size_t        JSON_DOC_CAPACITY   = 4096;
...
void dumpRequestTable() {
  ...
  for (int f = 0; f < NUM_FLOORS; f++) if (cabinRequests[f]) Serial.print(f);
  Serial.print(" up=");
  for (int f = 0; f < NUM_FLOORS; f++) if (hallUpRequests[f]) Serial.print(f);
  Serial.print(" down=");
  for (int f = 0; f < NUM_FLOORS; f++) if (hallDownRequests[f]) Serial.print(f);
  Serial.print(" dir=");
  Serial.print(dispatchDirection > 0 ? "UP" : dispatchDirection < 0 ? "DOWN" : "IDLE");
...
// FAN STATE
// =====================================================
enum FanMode { FAN_MODE_AUTO, FAN_MODE_MANUAL };
...
// =====================================================
// RFID / SECURITY DISPLAY STATE
// -----------------------------------------------------
// The current sketch does not contain an MFRC522/RC522 reader driver yet.
// These state hooks keep the LCD and Ditto security feature ready for the
// future RFID module without inventing direct floor-control authority here.
// A real RFID handler should call recordRfidEvent() after validating a card.
// =====================================================
...
char lastRfidUid[17] = "----";
char lastRfidRole[9] = "NONE";
char lastRfidReason[17] = "NO CARD";
bool lastRfidAuthorized = false;
bool securityLocked = false;
unsigned long lastRfidEventMs = 0;
const unsigned long RFID_STATUS_HOLD_MS = 10000;
...
void recordRfidEvent(bool authorized, const char* role, const char* uid, const char* reason) {
  ...
  lastRfidAuthorized = authorized && !securityLocked;
  ...
  if (authorized && !securityLocked) {
    ...
    lcdShowMessageTemporary("ACCESS GRANTED", roleLine, uidLine, "Session active", LCD_TEMP_MESSAGE_DURATION_MS);
  } else {
...
void setSecurityLockdown(bool locked, const char* source) {
  ...
  if (source && source[0]) { Serial.print(" ("); Serial.print(source); Serial.print(")"); }
  Serial.println();
}
...
void fanOn()  { markFanActivity(); setFanState(true,  "ACTIVITY"); }
void fanOff() { setFanState(false, "IDLE"); }
...
void setupSimulatedTelemetryAdc() {
  ...
  Serial.print("[ADC] TEMP GPIO"); Serial.print(SIM_TEMP_ADC_PIN);
  Serial.print(" range "); Serial.print(SIM_TEMP_MIN_C, 1);
  Serial.print(".."); Serial.print(SIM_TEMP_MAX_C, 1); Serial.println(" C");
  Serial.print("[ADC] VIB  GPIO"); Serial.print(SIM_VIB_ADC_PIN);
  Serial.print(" range "); Serial.print(SIM_VIB_MIN_G, 3);
  Serial.print(".."); Serial.print(SIM_VIB_MAX_G, 3); Serial.println(" g");
  Serial.print("[ADC] LOAD GPIO"); Serial.print(SIM_LOAD_ADC_PIN);
  Serial.print(" range "); Serial.print(SIM_LOAD_MIN_KG, 0);
  Serial.print(".."); Serial.print(SIM_LOAD_MAX_KG, 0); Serial.println(" kg");
}
...
void requestBuzzerPattern(uint8_t pattern) {
  if (!ENABLE_BUZZER || pattern == BUZZ_PATTERN_NONE) return;
  if (activeBuzzerPattern != BUZZ_PATTERN_NONE &&
      buzzerPatternPriority(activeBuzzerPattern) > buzzerPatternPriority(pattern)) {
    return;
  }
...
bool warningConditionNow() {
  ...
  return simulatedLoadKg >= SIM_LOAD_RATED_KG ||
         simulatedTemperatureC >= SIM_TEMP_WARNING_C ||
         simulatedVibrationG >= SIM_VIB_WARNING_G;
}
...
void updateBuzzer() {
  ...
  if (!inFaultState() && warningAlertConfirmed) {
    if (activeBuzzerPattern != BUZZ_PATTERN_WARNING &&
        buzzerPatternPriority(activeBuzzerPattern) <= buzzerPatternPriority(BUZZ_PATTERN_WARNING)) {
...
void stopStepper() { pinMode(STEP_PIN, INPUT); }
...
void doorOpenMotor()  { doorDrive(true); }
void doorCloseMotor() { doorDrive(false); }
...
bool startMovementSegmentTowardTarget() {
  ...
  if (movingUp) { setDirectionUp();   transitionTo(ST_MOVING_UP); }
  else          { setDirectionDown(); transitionTo(ST_MOVING_DOWN); }
...
void handleRequest(int floor, RequestType type, const char* source) {
  ...
  if (state == ST_IDLE) {
    ...
    bool ahead = (dispatchDirection > 0 && floor > currentFloor) ||
                 (dispatchDirection < 0 && floor < currentFloor);
...
static bool redirectTrip(int newTargetFloor, const char* reason) {
  ...
  bool ahead = (dispatchDirection > 0 && newTargetFloor > currentFloor) ||
               (dispatchDirection < 0 && newTargetFloor < currentFloor);
  ...
  // the planned stop.
  bool closerThanTarget = (dispatchDirection > 0 && newTargetFloor <= targetFloor) ||
                          (dispatchDirection < 0 && newTargetFloor >= targetFloor);
  ...
  maxMoveTimeMs      = (millis() - moveStartMs) +
                       (unsigned long)MAX_FLOOR_TRAVEL_MS * floorsAhead;
...
void tickMoving() {
  ...
  if (stepsSinceLastBoundary >= (unsigned long)STEPS_PER_FLOOR
    ...
    Serial.print("[MOVE] crossing floor ");
    Serial.println(currentFloor);
...
void handleSerialCommands() {
  ...
  if (c >= '0' && c <= '3') { handleFloorRequest(c - '0', "SERIAL"); return; }
  ...
  if (c == 's' || c == 'S') { enterErrorStop("Manual stop from serial monitor."); return; }
  if (c == 'e' || c == 'E') { enterEmergency("Manual emergency from serial monitor."); return; }
  if (c == 'q' || c == 'Q') {
    dumpRequestTable();
    return;
  }
...
void printLcdConfiguration() {
  ...
  Serial.print("  ENABLE_LCD       : "); Serial.println(ENABLE_LCD ? "true" : "false");
  Serial.print("  Config address   : 0x"); Serial.println(LCD_I2C_ADDRESS, HEX);
  Serial.print("  Active address   : 0x"); Serial.println(lcdActiveAddress, HEX);
  Serial.print("  Geometry         : "); Serial.print(LCD_COLS); Serial.print("x"); Serial.println(LCD_ROWS);
  Serial.print("  SDA/SCL GPIO     : "); Serial.print(LCD_SDA_PIN); Serial.print(" / "); Serial.println(LCD_SCL_PIN);
  Serial.print("  I2C clock        : "); Serial.print(LCD_I2C_CLOCK_HZ); Serial.println(" Hz");
  Serial.print("  Update interval  : "); Serial.print(LCD_UPDATE_INTERVAL_MS); Serial.println(" ms");
  Serial.print("  Update in moving : "); Serial.println(LCD_UPDATE_WHILE_MOVING ? "yes" : "no");
  Serial.print("  LCD ready        : "); Serial.println(lcdReady ? "yes" : "no");
...
void lcdShowRFID() {
  ...
  char line3[LCD_COLS + 1];
  char line4[LCD_COLS + 1];
  snprintf(line2, sizeof(line2), "Status:%s", rfidStateLabel());
  snprintf(line3, sizeof(line3), "Role:%s", lastRfidRole);
  snprintf(line4, sizeof(line4), "UID:%s", lastRfidUid);
  lcdSetLines("RFID SECURITY", line2, line3, line4);
}
...
void setup() {
  ...
  Serial.println("============================================");
  Serial.print("Floors                 : "); Serial.println(NUM_FLOORS);
  Serial.print("STEPS_PER_FLOOR        : "); Serial.println(STEPS_PER_FLOOR);
  Serial.print("STEP_ACCEL_STEPS       : "); Serial.println(STEP_ACCEL_STEPS);
  Serial.print("STEP_DECEL_STEPS       : "); Serial.println(STEP_DECEL_STEPS);
  Serial.print("STEP_DELAY_FAST_US     : "); Serial.println(STEP_DELAY_FAST_US);
  Serial.print("STEP_DELAY_SLOW_US     : "); Serial.println(STEP_DELAY_SLOW_US);
  Serial.print("MAX_FLOOR_TRAVEL_MS    : "); Serial.println(MAX_FLOOR_TRAVEL_MS);
  Serial.print("Door pulse / dwell ms  : "); Serial.print(DOOR_PULSE_MS); Serial.print(" / "); Serial.println(DOOR_DWELL_MS);
  Serial.print("DOOR_INVERT            : "); Serial.println(DOOR_INVERT ? "true" : "false");
  Serial.print("DIR_UP_ACTIVE_LOW      : "); Serial.println(DIR_UP_ACTIVE_LOW ? "true" : "false");
  Serial.print("EMERGENCY_STOP_PIN     : "); Serial.println(EMERGENCY_STOP_PIN);
  Serial.println("Dispatch logic         : collective selective control");
  Serial.print("Fan relay GPIO         : "); Serial.println(FAN_RELAY_PIN);
  ...
  Serial.print("ADC safety interlock   : "); Serial.println(ENABLE_ADC_SAFETY_INTERLOCK ? "ENABLED" : "DISABLED");
  Serial.print("ADC safety confirm ms  : "); Serial.println(ADC_SAFETY_FAULT_CONFIRM_MS);
  Serial.print("LCD HMI                : "); Serial.println(ENABLE_LCD ? "ENABLED" : "DISABLED");
  Serial.print("LCD SDA/SCL GPIO       : "); Serial.print(LCD_SDA_PIN); Serial.print(" / "); Serial.println(LCD_SCL_PIN);
  Serial.print("LCD configured address : 0x"); Serial.println(LCD_I2C_ADDRESS, HEX);
  ...
  {
    Serial.print("[TIME] NTP sync for TLS");
    ...
    Serial.println(nowSec < 1700000000UL ? " NOT synced (TLS may fail until time is set)" : " ok");
  }
  ...
  Serial.print  ("Commands topic  : "); Serial.println(MQTT_COMMANDS_TOPIC);
  Serial.println();
