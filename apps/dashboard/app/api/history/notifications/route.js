...
export async function GET(request) {
  ...
  try {
    ...
    const rows = await queryRows(
      `SELECT id, created_at, sent_at, severity, channel, status,
              attempts, priority, last_error,
              COALESCE(payload->>'message', payload->>'compact') AS message,
              COALESCE(payload->>'subject', payload->>'title') AS subject
    ...
    console.error("[api/history/notifications]", err.message);
