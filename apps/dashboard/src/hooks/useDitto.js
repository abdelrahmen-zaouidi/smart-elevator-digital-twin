...
export function useDitto(options = {}) {
  ...
  // Keep the latest onThingUpdate callback in a ref WITHOUT writing during
  // render — React disallows ref mutation in the render phase.
  useEffect(() => {
  ...
  useEffect(() => {
    if (!enabled) {
      // Disabled: tear down the transport and let the gated return below expose
      // defaults. No setState here, so the effect never cascades a render.
  ...
  // Gate the exposed values by `enabled` so a disabled hook reports defaults
  // without needing to setState inside the effect when it is torn down.
  return {
    thing: enabled ? thing : null,
    isConnected: enabled ? isConnected : false,
    mode: enabled ? mode : "idle",
