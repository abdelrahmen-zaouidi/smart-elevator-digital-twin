...
// ── Page components ──────────────────────────────────────────────────────────
// Extracted from the ElevatorOS monolith (stage c.2). Each page reads live state
// via useTwin() and is registered in PAGE_MAP back in the shell. The import set
// mirrors the monolith's (pages used a subset); unused entries are harmless.
...
import {
  Activity, AlertTriangle, BarChart3, Bell, BookOpen, ChevronLeft, ChevronRight, CircleHelp,
  Clock, Ban, Cpu, Database, DoorClosed, DoorOpen, Download, Eye, EyeOff, FileText, Gauge,
  KeyRound, LayoutDashboard, ListX, Loader2, LogOut, Menu, Monitor, Moon, PanelLeftClose,
  PanelLeftOpen, Pencil, Plus, Power, RadioTower, RefreshCw, Save, Search, Settings, Shield,
  SlidersHorizontal, Sun, Terminal, Trash2, User, Wrench, X,
} from "lucide-react";
...
import CommandSafetyGatePanel from "@/components/scada/CommandSafetyGatePanel";
import DispatchPolicyPanel from "@/components/scada/DispatchPolicyPanel";
import AgentActivityPanel from "@/components/scada/AgentActivityPanel";
...
import {
  Card, KpiTile, StatusPill, SevBadge, EmptyState, FieldTile, ConnectionIndicator, CmdBtn,
  RiskGauge, TelemetryChart, HealthBar, FanControlCard, ToastStack, GlobalAlertBanner,
  TableShell, MiniIconButton, ToggleSwitch, SettingsSection, ConfirmModal,
...
// Page: AI analytics
export function PageAIInsights() {
...
export const ACCESS_DECISION_COLOR = (decision) => {
...
export function AccessDecisionBadge({ decision }) {
...
export const EMPTY_TAG_DRAFT = { uid: "", label: "", role: "VISITOR", note: "", enabled: true, floors: [0, 1, 2, 3] };
...
// Page: Simulation lab
export function PageSimulation() {
...
export function buildAlertRows(state) {
  const cabin = state.features.cabin.properties;
  const door = state.features.door.properties;
  const motor = state.features.motor.properties;
  const sec = state.features.security.properties;
