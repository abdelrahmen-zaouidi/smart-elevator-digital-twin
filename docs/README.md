...
| [operations.md](operations.md) | Backup/restore/DR, TimescaleDB hypertable cutover, structured-log correlation, the Prometheus/Grafana observability profile, and per-user auth/RBAC setup. |
| [api/openapi.yaml](api/openapi.yaml) | OpenAPI 3.1 spec for the dashboard API (commands, health, metrics, access-control, history); validated in CI (`npm run openapi:lint`). |
