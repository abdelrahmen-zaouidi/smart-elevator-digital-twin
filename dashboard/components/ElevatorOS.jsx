/**
 * ╔══════════════════════════════════════════════════════════════════════╗
 * ║  ELEVATOROS v3.0 - ENTERPRISE SCADA DIGITAL TWIN COMMAND CENTER     ║
 * ║  Agentic AI Smart Elevator Management System                         ║
 * ║  Production-grade industrial control room interface                  ║
 * ╚══════════════════════════════════════════════════════════════════════╝
 *
 * Architecture:
 *  - TwinContext (global state via Context API)
 *  - useDigitalTwinEngine (master hook - telemetry, physics, commands)
 *  - Toast notification system
 *  - Confirmation dialog system
 *  - 7 fully-featured SCADA pages
 *  - Reusable design-system components
 *
 * Stack: React 18 + Recharts + Tailwind CSS
 * Compatible: Vite, Next.js, CRA
 */
...
import { updateAttributes, updateFeature } from "../src/services/dittoApi";
...
function useDigitalTwinEngine() {
  ...
  const injectInvalidRFIDCommand = useCallback(() => {
    const card = `UNKNOWN_${Math.floor(Math.random() * 9000) + 1000}`;
    commitState(previousState => addIncident({
      ...previousState,
      features: { ...previousState.features, security: { properties: { ...previousState.features.security.properties, rfid_last_card: card, rfid_access_granted: false, unauthorized_access_attempts: previousState.features.security.properties.unauthorized_access_attempts + 1, alert_level: "HIGH" }}},
      attributes: { ...previousState.attributes, risk_score: Math.min(100, previousState.attributes.risk_score + 18) },
    }, "UNAUTHORIZED_RFID", `Preview: card ${card} denied - not in whitelist`), "PREVIEW");
    logCmd("INJECT_INVALID_RFID", "PREVIEW", card);
    toast("WARNING", `Local preview: unauthorized RFID ${card} (will be overwritten by next Ditto update)`);
  }, [commitState, logCmd, toast]);
  ...
  return {
    ...
    emergencyStop: emergencyStopCommand, lockdown: triggerLockdownCommand, maintenance: maintenanceCommand, reset: resetSystemCommand, sendFloor: moveToFloorCommand, optimizeRouting, reduceEnergy, setFan: setFanCommand,
    ...
    <>
      ...
      >
        ...
        <span style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          <span>{label}</span>
          {disabled && reason && <span style={{ color: T.textMute, fontSize: 10, fontWeight: 500, letterSpacing: 0 }}>{reason}</span>}
        </span>
      </button>
      <ConfirmModal
        open={confirming}
...
// ─────────────────────────────────────────────────────────────────────────────
function PageTwin() {
  const { state, stateDiff, sendFloor, dittoConnected, dittoMode, connected, isSimulationMode, timeline, speedH, riskH } = useTwin();
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 18, alignItems: "center" }}>
          ...
          <div className="eos-status-list">
            ...
            <div className="eos-kpi-grid">
              ...
              <KpiTile label="Speed" value={c.speed_ms.toFixed(2)} unit="m/s" color={T.cyan} />
      ...
      <div className="eos-responsive-grid three">
        ...
        <Card title="Motor Telemetry" accent={m.health_status === "GOOD" ? T.green : m.health_status === "WARNING" ? T.yellow : T.red}>
          <div className="eos-status-list">
            ...
            <TelemetryChart data={speedH} color={T.cyan} yDomain={[0, 4]} height={88} unit=" m/s" />
          </div>
        </Card>
...
// ─────────────────────────────────────────────────────────────────────────────
// PAGE 4 - SECURITY OPS CENTER (SOC)
// ─────────────────────────────────────────────────────────────────────────────
function PageSOC() {
    ...
    <>
      ...
      <aside className={`eos-sidebar ${collapsed ? "is-collapsed" : ""} ${mobileOpen ? "is-open" : ""}`}>
        <div className="eos-sidebar-brand">
          <div className="eos-brand-mark"><Zap size={18} /></div>
...
function PageControlPanel() {
  const { state, sendFloor, emergencyStop, lockdown, maintenance, reset, optimizeRouting, reduceEnergy, setFan, injectHighVib, injectForcedEntry, injectAudioDistress, injectInvalidRFID, runScenario, cmdLog, connected, dittoConnected } = useTwin();
  const cabin = state.features.cabin.properties;
  const motor = state.features.motor.properties;
  const fan = state.features.fan.properties;
  const attr = state.attributes;
    ...
    <div className="eos-page-stack">
      <div className="eos-responsive-grid three">
        ...
        <Card title="Safety Critical Commands" accent={T.red}>
          <div className="eos-command-list"><CmdBtn label="Emergency Stop" icon="STOP" onClick={emergencyStop} variant="danger" confirm disabled={cabin.emergency_stop} reason="Already active" /><CmdBtn label="Lockdown" icon="LOCK" onClick={lockdown} variant="danger" confirm disabled={attr.system_mode === "LOCKDOWN"} reason="Already locked" /><CmdBtn label="Maintenance Mode" icon="MX" onClick={maintenance} variant="warning" confirm disabled={attr.system_mode === "MAINTENANCE"} reason="Already in maintenance" /><CmdBtn label="Reset / Clear Problems" icon="RST" onClick={reset} variant="success" confirm /></div>
        </Card>
  ...
  return (
    <div className="eos-settings-grid">
      ...
      <SettingsSection title="Project Runtime" accent={T.green}>
        ...
        <FieldTile label="Ditto credentials" value={env.DITTO_USERNAME || env.DITTO_PASSWORD ? "Configured" : "Not configured"} color={T.textSub} />
  ...
  return (
    <div className="eos-login-shell">
      <form className="eos-login-card" onSubmit={submit}>
        <div className="eos-login-brand"><div className="eos-brand-mark large"><Zap size={24} /></div><div><h1>ElevatorOS</h1><p>Local Demo Lock Screen</p></div></div>
